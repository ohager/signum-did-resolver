...
describe('DidParser', () => {
  ...
  describe('parse()', () => {
    describe('valid DIDs', () => {
      ...
      it('should parse transaction DID with explicit mainnet', () => {
        const result = parser.parse('did:signum:mainnet:tx:12345678901234567890');
        expect(result).toEqual({
      ...
      it('should parse transaction DID with testnet', () => {
        const result = parser.parse('did:signum:testnet:tx:98765432109876543210');
        expect(result).toEqual({
    ...
    describe('invalid DIDs', () => {
      it('should throw on empty string', () => {
        ...
        expect(() => parser.parse('')).toThrow('DID must be a non-empty string');
      });

      it('should throw on DID without "did:" prefix', () => {
        expect(() => parser.parse('signum:tx:12345678901234567890')).toThrow(DidParseError);
      ...
      it('should throw on wrong method', () => {
        expect(() => parser.parse('did:ethereum:tx:12345678901234567890')).toThrow(DidParseError);
        expect(() => parser.parse('did:ethereum:tx:12345678901234567890')).toThrow(
          'Invalid Signum DID format'
        );
      });

      it('should throw on invalid network', () => {
        expect(() => parser.parse('did:signum:invalidnet:tx:12345678901234567890')).toThrow(
          DidParseError
        );
      });

      it('should throw on invalid type', () => {
        expect(() => parser.parse('did:signum:invalidtype:12345678901234567890')).toThrow(
          DidParseError
        );
      });
      ...
      it('should throw on empty identifier', () => {
        ...
        expect(() => parser.parse('did:signum:tx:')).toThrow('Invalid Signum DID format');
      });

      it('should throw on invalid transaction ID (too short)', () => {
        expect(() => parser.parse('did:signum:tx:12345')).toThrow(DidParseError);
        expect(() => parser.parse('did:signum:tx:12345')).toThrow('Must be 18-23 digit numeric ID');
      });

      it('should throw on invalid transaction ID (too long)', () => {
        expect(() => parser.parse('did:signum:tx:123456789012345678901234')).toThrow(
          DidParseError
        );
      });

      it('should throw on invalid transaction ID (non-numeric)', () => {
        expect(() => parser.parse('did:signum:tx:abc123def456')).toThrow(DidParseError);
      });

      it('should throw on invalid account RS-address format', () => {
        expect(() => parser.parse('did:signum:acc:INVALID-ADDRESS')).toThrow(DidParseError);
        expect(() => parser.parse('did:signum:acc:S-ABC')).toThrow(DidParseError);
      });

      it('should throw on invalid alias format', () => {
        expect(() => parser.parse('did:signum:alias:invalid@name')).toThrow(DidParseError);
        expect(() => parser.parse('did:signum:alias:tld:name:extra')).toThrow(DidParseError);
      });
    });
  ...
  describe('isValid()', () => {
    it('should return true for valid DIDs', () => {
      ...
      expect(parser.isValid('did:signum:testnet:acc:S-9K9L-4CB5-88Y5-F5G4Z')).toBe(true);
      expect(parser.isValid('did:signum:acc:TS-9K9L-4CB5-88Y5-F5G4Z')).toBe(true);
