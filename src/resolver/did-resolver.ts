...
import {TransactionDidDocumentBuilder, AccountDidDocumentBuilder} from '@/builders';
import type {
    DidResolutionResult,
    ParsedDid,
    DIDResolutionError, Src44Data,
} from '@/types/did';
...
export class SignumDidResolver {
    ...
    async resolve(did: string): Promise<DidResolutionResult> {
        ...
        try {
            ...
            return this.createErrorResult('invalidDid', error instanceof Error ? error.message : 'Invalid DID format');
        }
        ...
        try {
            switch (parsedDid.type) {
                ...
                case 'contract':
                case 'token':
                    return this.createErrorResult('methodNotSupported', `DID type '${parsedDid.type}' not yet implemented`);
                default:
                    return this.createErrorResult('invalidDid', `Unknown DID type: ${parsedDid.type}`);
            }
        } catch (error) {
            return this.createErrorResult('internalError', error instanceof Error ? error.message : 'Unknown error');
        }
    }
     ...
     */
    private async resolveTransaction(parsedDid: ParsedDid): Promise<DidResolutionResult> {

        try {
            // Fetch transaction from blockchain
            const tx = await this.ledgerClient.transaction.getTransaction(parsedDid.identifier);
            ...
            if (error instanceof Error && error.message.includes('Unknown transaction')) {
                return this.createErrorResult('notFound', `Transaction ${parsedDid.identifier} not found`);
            }
            throw error;
        }
    }
     ...
     */
    private async resolveAccount(parsedDid: ParsedDid): Promise<DidResolutionResult> {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('Unknown account')) {
                return this.createErrorResult('notFound', `Account ${parsedDid.identifier} not found`);
            }
            throw error;
        }
    }
     ...
     */
    private createErrorResult(error: DIDResolutionError, message: string): DidResolutionResult {
        return {
            didResolutionMetadata: {
                error,
                message,
